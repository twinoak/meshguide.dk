...
export function createFlow({ ui, log, setStatus, mode = "direct", onReboot = async () => {}, reread = null }) {
  ...
  async function showCompanion(identity) {
    ...
    identity.autoAdd = await link.companionGetAutoAdd();
    ...
    showError(ui.deviceNote, "Dette er companion-firmware (den der bruges sammen med appen). Den har ingen CLI, så repeater-opsætningen gælder ikke - men de få indstillinger der betyder noget for det danske mesh - path.hash.mode, standard-scope (#dk) og at den ældste kontakt overskrives når listen er fuld - kan tjekkes og rettes herunder.");
