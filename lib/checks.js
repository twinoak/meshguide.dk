...
export const DEFAULTS_ANCHOR = Object.freeze({
  ...
  cad: "def-cad", "radio.rxgain": "def-rxgain", "radio.fem.rxgain": "def-rxgain", "agc.reset.interval": "def-agc.reset.interval",
...
export function parseState(replies) {
  ...
  return {
    ...
    cad: onOff(r.cad),                          // "on" | "off" | UNSUPPORTED | null
    rxGain: onOff(r.rxGain),
...
export function evaluate(state, location, scopes, input = {}) {
  ...
  // --- cad, rxgain, fem rxgain, agc.reset.interval ----------------------------
  ...
  setting("cad", "cad", state.cad, BP.cad);
...
export function planCommands(findings, selectedIds) {
  const order = ["location", "region.default", "regions", "regions.extra", "owner.info", "dutycycle", "path.hash.mode", "contacts.overwrite", "advert.interval", "flood.advert.interval", "guest.password", "loop.detect", "flood.max.unscoped", "cad", "radio.rxgain", "radio.fem.rxgain", "agc.reset.interval", "rxdelay", "txdelay", "direct.txdelay", "radio"];
