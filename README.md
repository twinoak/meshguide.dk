...
- **Sæt en companion op** - over USB eller Bluetooth. De to indstillinger der betyder noget for det danske mesh (`path.hash.mode` og standard-scope `#dk`) tjekkes og rettes.
...
- `lib/flow.js` er den fælles del af UI'et - læs indstillinger, kort/placering, anbefalinger, anvend - uafhængigt af hvordan enheden nås. Den får et "link" med `command(cmd)`: enten USB/Bluetooth-forbindelsen selv, eller en `RemoteCli`. Én flow-instans skifter regler med `setMode("direct" | "remote")`.
