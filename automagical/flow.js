...
// selected changes. index.html drives it over a direct link (USB / Bluetooth),
// remote.html over a companion's radio (remote-cli.js). A link is anything
// with command(cmd, opts) returning the shape of MeshCoreSerial.command().

import { buildDataset, scopesForPoint } from "../scopes.js";
import { BEST_PRACTICE, READ_COMMANDS, commandText, evaluate, evaluateCompanion, forwards, hasDeviceLocation, isRepeater, lockFindings, needsReboot, parseState, planCommands, roleLabel, scopeKeyFor } from "./checks.js";
...
// --- The flow ------------------------------------------------------------------------
//
// ui: the elements listed in REQUIRED_UI (same ids on both pages).
...
//       index.html re-identifies first, so a companion's DEVICE_INFO is fresh too).
...
export function createFlow({ ui, log, setStatus, mode = "direct", onReboot = async () => {}, reread = null }) {
  ...
  // Switches between direct and remote wording/rules at runtime (v2 keeps one flow
  ...
  async function readSettingsSteps(link) {
    ...
    setStatus((remote ? "Logget ind på: " : "Forbundet: ") + roleLabel(app.state.role) + " · " + (app.state.name || "(uden navn)") + " · " + (app.state.board || "") + " · " + (app.state.version ? app.state.version.text : ""), "connected");
    renderDevice();
    const notes = [];
  ...
  async function showCompanion(identity) {
    ...
    const s = identity.selfInfo;
    setStatus("Forbundet" + (link.kind === "ble" ? " via Bluetooth" : "") + ": companion" + (s && s.name ? " · " + s.name : "") + (identity.deviceInfo ? " · " + identity.deviceInfo.board + " · " + (identity.deviceInfo.firmwareVersion || "") : ""), "connected");
